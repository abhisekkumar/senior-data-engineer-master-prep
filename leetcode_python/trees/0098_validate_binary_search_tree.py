...
Restate the Problem:
    Explain the input, expected output, and objective for Validate Binary Search Tree before coding.
...
Optimal Approach:
    Apply the dfs bounds pattern while maintaining its invariant.
...
Every node: Left subtree  <  Node  <  Right subtree
...
We need to check against all of the ancestors, not just the parent.
"""
